...
template <typename T> T MessageQueue<T>::receive() {
  ...
  // _conditio.wait() to wait for and receive new messages and pull them from
...
void TrafficLight::waitForGreen() {
  ...
  // in which an infinite while-loop
...
void TrafficLight::cycleThroughPhases() {
  ...
  auto earlycycleTime = std::chrono::system_clock::now();

  while (true) {
    std::this_thread::sleep_for(std::chrono::milliseconds(1));
    auto timeBetween = std::chrono::duration_cast<std::chrono::milliseconds>(
        std::chrono::system_clock::now() - earlycycleTime);
    int durationBewteen = timeBetween.count();

    if (durationBewteen >= cycleDuration) {
      _currentPhase = _currentPhase == green ? red : green;
      std::future<void> sendFuture =
          std::async(&MessageQueue<TrafficLightPhase>::send, &_messages,
                     std::move(_currentPhase));
      sendFuture.wait();
      earlycycleTime = std::chrono::system_clock::now();
